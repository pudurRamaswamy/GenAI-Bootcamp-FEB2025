    ...
    if vAR_api_key:
        ...
        if message == "Valid API Key!":
            with col2:
                ...
                st.write("### ")
                st.markdown(
                    "<p style='text-align: left; color: black; font-size:20px;'><span style='font-weight: bold'>Enter System Instruction</span></p>", 
                    unsafe_allow_html=True
                )
            with col3:
                st.write("### ")
                vAR_system_instr = st.text_area(" ", placeholder="You are a helpful assistant to answer user queries" ,key="system_Instruction")
            if vAR_system_instr:
                text_based(vAR_api_key, vAR_system_instr)

    ...
    if vAR_api_key:
        ...
        if message == "Valid API Key!":
            with col2:
                ...
                st.write("### ")
                st.markdown(
                    "<p style='text-align: left; color: black; font-size:20px;'><span style='font-weight: bold'>KnowledgeBase Type</span></p>", 
                    unsafe_allow_html=True
                )
            with col3:
                st.write("### ")
                vAR_kb_type = st.selectbox(" ",["Select", "PDF", "CSV"])
            if vAR_kb_type == "PDF":
                with col2:
                    st.write('## ')
                    st.write("### ")
                ...
                with col3:
                    st.write("### ")
                    uploaded_file  = st.file_uploader("", type=['pdf'], key="fileupload")
                if uploaded_file :
                    vAR_DB=vectorstore(uploaded_file, vAR_api_key)
                    text_based(vAR_api_key, vAR_DB, vAR_kb_type)
            ...
            if vAR_kb_type == "CSV":
                ...
                with col3:
                    st.write("### ")
                    uploaded_file  = st.file_uploader("", type='csv', key="fileupload_2")
                if uploaded_file:
                    vAR_csvagent=csv_agent(uploaded_file, vAR_api_key)
                    text_based(vAR_api_key, vAR_csvagent, vAR_kb_type)
